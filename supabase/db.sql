...
CREATE OR REPLACE FUNCTION public.scan_attendance(
    ...
    p_scan_face_image_path TEXT DEFAULT NULL 
)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY INVOKER 
...
DECLARE
  v_active_session_id BIGINT;
  v_debounce_interval INTERVAL := interval '1 minute'; 
...
CREATE OR REPLACE FUNCTION public.get_attendance_logs_with_details(
    ...
    p_scan_status_filter public.scan_status DEFAULT NULL 
)
RETURNS TABLE (
    log_id BIGINT,
...
BEGIN
  ...
  WHERE
    ...
    AND
    (p_scan_status_filter IS NULL OR al.status = p_scan_status_filter) 
  ORDER BY
    al.scanned_at DESC;
END;
$$;
...
BEGIN
    ...
    WITH relevant_student_sessions AS (
        ...
        WHERE sess.class_name = v_student_record.class_name 
    ),
